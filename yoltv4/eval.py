...
Evaluate f1/map scores of predictions compared to ground truth

Adapted from: https://github.com/avanetten/simrdwn/blob/master/simrdwn/core/simrdwn_eval.py
...
def execute(args):
    ...
    # ground truth
    cat_col_truth = 'make'
    im_name_col = 'im_name_root'
    
    gdf_truth = gpd.read_file(args.truth_file)
    print("gdf_truth.columns:", gdf_truth.columns)
    print("len(gdf_truth):", len(gdf_truth))

    im_roots = np.unique(gdf_truth[im_name_col])
    ...
    categories = np.sort(np.unique(gdf_truth[cat_col_truth]))
    print("ground truth categories:", categories)

    unique, counts = np.unique(
        gdf_truth[cat_col_truth].values, return_counts=True)
    print("Ground Truth Category counts")
    print(np.asarray((unique, counts)).T)

    ##############
    # proposals
    cat_col_prop = 'category'
    prob_col = 'prob'
    
    gdf_prop = gpd.read_file(args.prop_file)
    print("gdf_prop.columns:", gdf_prop.columns)
    print ("len(gdf_prop):", len(gdf_prop)

    unique, counts = np.unique(df_prop[cat_col_prop].values, return_counts=True)
    ...
    # get minimum prop
    min_prob = np.min(df_prop[prob_col].values)
    ...
    if args.run_prop_test == 1:
        ...
        print("im_roots:", im_roots)
        groupby = 'im_name_root'  # 'Image_Root'
        groupby_cat = cat_col_prop
        ...
        while not selected_im:
            ...
            # get truth subset
            gdf_truth_filt = gdf_truth[(gdf_truth[im_name_col] == im_root)]
            cats_present_truth = np.unique(gdf_truth_filt[cat_col_truth].values)
            #category = categories[np.random.randint(len(categories))]
            print("cats_present_truth:", cats_present_truth)

            # get proposal subset
            df_prop_filt = df_prop[(df_prop[im_name_col] == im_root) &
                                   (df_prop[prob_col] >= args.detect_thresh_plot)]
            cats_present_prop = np.unique(df_prop_filt[cat_col_prop].values)
